  ...
  raw_df = raw_df %>%
    dplyr::mutate(
      penalty_detail = case_when(
        ...
        TRUE ~ NA_character_),
      penalty_play_text = ifelse(.data$penalty_flag, str_extract(.data$play_text, regex("Penalty(.+)",ignore_case = TRUE)), NA_character_),
      yds_penalty = ifelse(.data$penalty_flag,
                           str_extract(.data$penalty_play_text, "(.{0,3})yards to the |(.{0,3})yds to the |(.{0,3})yd to the "), NA_real_),
      yds_penalty = str_remove(.data$yds_penalty, " yards to the | yds to the | yd to the "),
      yds_penalty = ifelse(.data$penalty_flag & str_detect(.data$play_text, "ards\\)") & is.na(.data$yds_penalty),
                           str_extract(.data$play_text, "(.{0,4})yards\\)|(.{0,4})Yards\\)|(.{0,4})yds\\)|(.{0,4})Yds\\)"), .data$yds_penalty),
      yds_penalty = str_remove(.data$yds_penalty, "yards\\)|Yards\\)|yds\\)|Yds\\)"),
      yds_penalty = str_remove(.data$yds_penalty, "\\(")
    )
  suppressWarnings(
    raw_df <- raw_df %>% 
      mutate(yds_penalty = str_trim(.data$yds_penalty))
  )
